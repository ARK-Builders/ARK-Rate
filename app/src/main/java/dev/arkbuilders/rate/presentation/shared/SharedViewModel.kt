...
class SharedViewModel(private val alertConditionRepo: PairAlertConditionRepo,
        private val quickBaseCurrencyRepo: QuickBaseCurrencyRepo) : ViewModel() {
    ...
    fun onAlertConditionCodePicked(code: CurrencyCode,
            numeratorNotDenominator: Boolean,
            conditionId: Long) {
        if (conditionId == 0L) {
            newCondition = if (numeratorNotDenominator) newCondition.copy(numeratorCode = code)
            else newCondition.copy(denominatorCode = code)
        ...
        val updatedCondition = if (numeratorNotDenominator) oldCondition.copy(numeratorCode = code)
        else oldCondition.copy(denominatorCode = code)
    ...
    fun onConditionMoreLessChanged(condition: PairAlertCondition) {
        ...
        val updatedCondition = condition.copy(moreNotLess = !condition.moreNotLess)
        pairAlertConditions[conditionIndex] = updatedCondition
        viewModelScope.launch {
            alertConditionRepo.insert(updatedCondition)
        }
    ...
    fun onRatioChanged(condition: PairAlertCondition, oldInput: String, newInput: String): String {
        ...
        viewModelScope.launch {
            ...
            val conditionIndex = pairAlertConditions.indexOf(condition)
            val updatedCondition = condition.copy(ratio = ratio)
            pairAlertConditions[conditionIndex] = updatedCondition
            alertConditionRepo.insert(updatedCondition)
        }
...
@Singleton
class SharedViewModelFactory @Inject constructor(private val alertConditionRepo: PairAlertConditionRepo,
        private val quickBaseCurrencyRepo: QuickBaseCurrencyRepo) : ViewModelProvider.Factory {
    override fun <T : ViewModel> create(modelClass: Class<T>): T {
        return SharedViewModel(alertConditionRepo, quickBaseCurrencyRepo) as T
    }
}